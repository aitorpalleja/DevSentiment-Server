...
export const getAndInsertPHPTweets = async (next_token) => {
  const query = {
    "query": "php programmer OR php programming OR php library OR php dev OR php developer lang:en -\"Essaydue\" -\"paywrite\" -\"essaypay\" -\"homeworkdue\" -\"assignmentdue\" -\"assignment due\" -\"essay pay\" -\"Essay due\" -\"pay write\" -\"Essays\"",
    "max_results": "100",
  ...
  originalTweets.forEach(async tweet => {
    await limiter.schedule(async () => {
      ...
      if (classifications) {
        ...
        const topicInDb = await Topic.findOne({ topic: "PHP" });
        if (topicInDb) {
  ...
  if (response.data.meta.next_token) {
    getAndInsertPHPTweets(response.data.meta.next_token);
  }
}
