...
export const getAndInsertReactjsTweets = async (next_token) => {
  const query = {
    "query": "react js OR react js programming OR react js library OR react js javascript OR learn react js lang:en -\"Essaydue\" -\"paywrite\" -\"essaypay\" -\"homeworkdue\" -\"assignmentdue\" -\"assignment due\" -\"essay pay\" -\"Essay due\" -\"pay write\" -\"Essays\"",
    "max_results": "100",
  ...
  originalTweets.forEach(async tweet => {
    await limiter.schedule(async () => {
      ...
      if (classifications) {
        ...
        const topicInDb = await Topic.findOne({ topic: "ReactJS" });
        if (topicInDb) {
  ...
  if (response.data.meta.next_token) {
    getAndInsertReactjsTweets(response.data.meta.next_token);
  }
}
