...
export const getAndInsertKotlinTweets = async (next_token) => {
  const query = {
    "query": "Kotlin lang:en -\"Essaydue\" -\"paywrite\" -\"essaypay\" -\"homeworkdue\" -\"assignmentdue\" -\"assignment due\" -\"essay pay\" -\"Essay due\" -\"pay write\" -\"Essays\"",
    "max_results": "100",
  ...
  originalTweets.forEach(async tweet => {
    await limiter.schedule(async () => {
      ...
      if (classifications) {
        ...
        const topicInDb = await Topic.findOne({ topic: "Kotlin" });
        if (topicInDb) {
  ...
  if (response.data.meta.next_token) {
    getAndInsertKotlinTweets(response.data.meta.next_token);
  }
}
