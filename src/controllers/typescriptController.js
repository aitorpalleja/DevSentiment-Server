...
export const getAndInsertTypescriptTweets = async (next_token) => {
  const query = {
    "query": "typescript lang:en -\"Essaydue\" -\"paywrite\" -\"essaypay\" -\"homeworkdue\" -\"assignmentdue\" -\"assignment due\" -\"essay pay\" -\"Essay due\" -\"pay write\" -\"Essays\"",
    "max_results": "100",
  ...
  originalTweets.forEach(async tweet => {
    await limiter.schedule(async () => {
      ...
      if (classifications) {
        ...
        const topicInDb = await Topic.findOne({ topic: "TypeScript" });
        if (topicInDb) {
  ...
  if (response.data.meta.next_token) {
    getAndInsertTypescriptTweets(response.data.meta.next_token);
  }
}
