...
export const getAndInsertSolidityTweets = async (next_token) => {
  const query = {
    "query": "Solidity lang:en -\"Essaydue\" -\"paywrite\" -\"essaypay\" -\"homeworkdue\" -\"assignmentdue\" -\"assignment due\" -\"essay pay\" -\"Essay due\" -\"pay write\" -\"Essays\"",
    "max_results": "100",
  ...
  originalTweets.forEach(async tweet => {
    await limiter.schedule(async () => {
      ...
      if (classifications) {
        ...
        const topicInDb = await Topic.findOne({ topic: "Solidity" });
        if (topicInDb) {
  ...
  if (response.data.meta.next_token) {
    getAndInsertSolidityTweets(response.data.meta.next_token);
  }
}
