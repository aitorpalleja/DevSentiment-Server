...
export const getAndInsertCSharpTweets = async (next_token) => {
  const query = {
    "query": "c# programming language OR c sharp OR csharp OR c# dev OR c# developer OR c# programmer lang:en -\"Essaydue\" -\"paywrite\" -\"essaypay\" -\"homeworkdue\" -\"assignmentdue\" -\"assignment due\" -\"essay pay\" -\"Essay due\" -\"pay write\" -\"Essays\"",
    "max_results": "100",
  ...
  originalTweets.forEach(async tweet => {
    await limiter.schedule(async () => {
      ...
      if (classifications) {
        ...
        const topicInDb = await Topic.findOne({ topic: "C#" });
        if (topicInDb) {
  ...
  if (response.data.meta.next_token) {
    getAndInsertCSharpTweets(response.data.meta.next_token);
  }
}
